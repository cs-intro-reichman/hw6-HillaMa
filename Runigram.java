...
public class Runigram {
	...
	public static void main(String[] args) {
		...
		image = scaled(tinypic, tinypic[0].length / 2, tinypic.length / 2);
		System.out.println("\nScaled to half the size:");
		print(image);

		image = scaled(tinypic, tinypic[0].length * 2, tinypic.length * 2);
		System.out.println("\nScaled to double the size:");
		print(image);

		Color[][] flippedImage = flippedHorizontally(tinypic);
		image = blend(tinypic, flippedImage, 0.5);
		System.out.println("\nBlended with horizontal flip (alpha = 0.5):");
		print(image);

		System.out.println("\nMorphing between original and flipped images:");
		morph(tinypic, flippedImage, 10); // Morph in 10 steps
	}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int row = (int)(i * ((double) ogHeight / height));
				int col = (int)(j * ((double) ogWidth / width));
